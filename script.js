...
const syncMobileQuoteLink = () => {
    if (!navLinks) return;

    const existingMobileQuote = navLinks.querySelector('.mobile-quote-link');
    if (existingMobileQuote) existingMobileQuote.remove();

    const quoteLink = document.querySelector('.nav-cta a');
    if (!quoteLink) return;

    const quoteItem = document.createElement('li');
    quoteItem.className = 'mobile-quote-link';

    const quoteClone = quoteLink.cloneNode(true);
    quoteClone.classList.add('mobile-quote-button');
    quoteClone.textContent = 'Get Quote';

    quoteItem.appendChild(quoteClone);
    navLinks.appendChild(quoteItem);
};
...
const initHeroModel = () => {
    ...
    // Add keyboard zoom support for all screens
    const handleKeyDown = (event) => {
        // Only handle zoom keys when globe is in focus or user is interacting
        if (event.ctrlKey || event.metaKey) {
            switch(event.key) {
                ...
                case 'ArrowUp':
                    event.preventDefault();
                    targetScale = Math.max(0.5, Math.min(3, targetScale + 0.2));
                    isDragging = true;
                    clearTimeout(window.zoomResetTimeout);
                    window.zoomResetTimeout = setTimeout(() => {
                        isDragging = false;
    ...
    // Add double-click zoom for all screens
    const handleDoubleClick = (event) => {
        event.preventDefault();
        
        // Toggle between zoomed and original size
    ...
    // Add dedicated touch event handlers for mobile zoom
    ...
    const handleTouchStart = (event) => {
        if (event.touches.length === 2) {
            ...
            isZooming = true;
            isDragging = true; // Prevent auto-reset during zoom
        }
    };
    ...
    const handleTouchMove = (event) => {
        if (event.touches.length === 2 && isZooming) {
            ...
            if (touchStartDistance > 0) {
                ...
                targetScale = Math.max(0.5, Math.min(3, touchStartScale * scaleRatio));
            }
        }
    };
    ...
    const handleTouchEnd = (event) => {
        if (event.touches.length < 2) {
            ...
            // Auto-reset zoom after touch ends
            clearTimeout(window.zoomResetTimeout);
            window.zoomResetTimeout = setTimeout(() => {
                targetScale = originalScale;
            }, 1500);
    ...
    try {
        ...
        model.addEventListener('dblclick', handleDoubleClick);
        
        // Add touch event listeners for mobile zoom
        model.addEventListener('touchstart', handleTouchStart, { passive: false });
        model.addEventListener('touchmove', handleTouchMove, { passive: false });
        model.addEventListener('touchend', handleTouchEnd, { passive: false });
        model.addEventListener('touchcancel', handleTouchEnd, { passive: false });
        
        // Add keyboard event listeners for global zoom control
...
if (contactForm && typeof THREE !== 'undefined') {
